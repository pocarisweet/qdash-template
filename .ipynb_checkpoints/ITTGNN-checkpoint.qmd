...
FiveVTS_random = pd.merge(df.query("dataset=='fivenodes' and mtype=='rand'").groupby(['model','mrate','nof_filters','inter_method','method','lags','epoch'])['mse'].mean().reset_index(),
         ...
         on=['model','mrate','inter_method','method','nof_filters','lags','epoch']).query("mrate in [0.3, 0.5, 0.6, 0.7, 0.8] and inter_method=='linear'").rename(columns={'model':'Model','mse_x':'Mean','mse_y':'SD','mrate':'Missing Rate','inter_method':'Interpolation','nof_filters':'Filters'}).assign(Method=lambda x: x['method'].map({'STGCN': 'Classic', 'IT-STGCN': 'Proposed','GNAR':'GNAR'})).drop(columns=['method','epoch'])
