...
fivenodes_fig = df[filter_condition & df['model'].isin(model_conditions)]

model_order = ["GConvGRU", "GConvLSTM", "GCLSTM", "LRGCN", "DyGrEncoder", "EvolveGCNH", "EvolveGCNO", "TGCN", "DCRNN"]
fivenodes_fig['model'] = pd.Categorical(fivenodes_fig['model'], categories=model_order, ordered=True)
fivenodes_fig['method'] = pd.Categorical(fivenodes_fig['method'], categories=['STGCN', 'IT-STGCN'], ordered=True)

fill_colors = ["steelblue", "pink"]
fivenodes_fig['log_mse'] = np.log10(fivenodes_fig['mse'])
fivenodes_fig['mrate_method_interaction'] = factor(fivenodes_fig['mrate'].astype(str) + '_' + fivenodes_fig['method'].astype(str))

fivenodes_fig2 = (ggplot(fivenodes_fig, aes(x='mrate', y='log_mse', group='interaction(mrate, method)')) +
      geom_boxplot(aes(fill='method'), color='black', width=0.7, outlier_color='black', outlier_shape=2) +
      scale_fill_manual(values=fill_colors, labels=['Classic', 'Proposed']) +
      theme(strip_background=element_blank(), panel_grid=element_blank(),
            legend_position='bottom', axis_text_x=element_text(angle=0, hjust=0.5, vjust=0.5)) +
